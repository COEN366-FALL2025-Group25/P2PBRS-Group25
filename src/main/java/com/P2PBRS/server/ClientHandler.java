...
public class ClientHandler extends Thread {
	...
	private String processReplicateDone(String message) {
		...
		// Update backup plan and registry
		Optional<PeerNode> targetPeerNode = registry.getPeer(targetPeer);
		if (targetPeerNode.isPresent()) {
			...
			for (BackupManager.Plan plan : BackupManager.getInstance().getPlans().values()) {
				if (plan.fileName.equals(fileName) && plan.placement.containsKey(chunkId)) {
					// Update backup plan
					plan.placement.put(chunkId, targetPeerNode.get());
					
					// Update registry chunk tracking
					registry.registerChunkStorage(fileName, chunkId, targetPeer);
					
					System.out.println("Updated chunk placement: " + fileName + " chunk " + chunkId + " -> " + targetPeer);
					break;
				}
			}
		} else {
			System.err.println("ERROR: Target peer not found in registry: " + targetPeer);
		}
